//! Mobius's desktop shell. Boots the orchestration host, then serves the same
//! web bundle the browser runs from a local port into a native webview window.
//! Debug builds read `../dist` from disk so a fresh `trunk build` shows up on
//! relaunch; release builds embed the bundle into the executable.
...
fn main() {
    if Dist::get("index.html").is_none() {
        eprintln!("the web bundle is missing, build it first with `just build`");
        std::process::exit(1);
    }
    mobius::launch(mobius::HostConfig::default());
    let port = serve_dist();
